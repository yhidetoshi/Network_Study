...
- ARP(Address Resolution Protocol)
  ...
  -  ARPによって取得した情報はARPテーブルにしばらくキャッシュされるので頻繁にブロードキャストされるのを防ぐ
  -  ARPメッセージはIPヘッダを付加せず、レイヤー2のヘッダとトレーラによって直接カプセル化される
  -  ARPリクエストのLayer2ヘッダ内のMACアドレスフィールドには[FF-FF-FF-FF-FF-FF]が含まれる
