...
-　ポーリングトトラップ
  - **ポーリング**
    - SNMPマネージャが定期的に自動監視を実行する
  - **トラップ** 
    - SNMPエージェントが自発的に情報を送信する。イベント発生時にTrapを送信してマネージャ側に通知する 
      - インタフェースのダウン
      - CPU使用率の閾値の超過
      - 機器の再起動
      - リンク障害によるダウン、回復
      - 認証ミス
      - ルーティングネイバーの関係喪失
    - SNMPv1(確認応答なし)、inform:SNMPv2(確認応答あり) 
...
- SNMPのバージョンによる違い
  - SNMPv2で追加されたコマンド
    - (コマンド)GetBulkRequest
      - (方向)マネージャ => エージェント 
        - (説明)高速に複数の情報を取得するときに使用
    - (コマンド)InformRequest
      - (方向)エージェント => マネージャ
        - (説明)マネージャへの認証応答を要求
  - SNMPv3 
    - セキュリティ機能の強化
      - **メッセージの完全性**: メッセージが改ざんされてないことを保証する
      - **認証**：メッセージの送信元の認証を行う
      - **機密性**：暗号化によって盗聴から情報を守る
...
- syslogサーバへログを飛ばす設定
#logging <ip-address>コマンド
```

- **NetFlow**
  - ルータを通過するトラフィックの統計情報をフロー単位で効率的に収集することができる
    - NWを利用するアプリケーションやユーザの継続的な監視
